<script lang="ts">
	...
	// Props
	let { updateSort, initialSort }: {
		updateSort: (sort: { field: string; order: 'asc' | 'desc' } | null) => void;
	...
	const initializeFromProps = () => {
		if (initialSort && !initialized) {
			sortField = initialSort.field;
			sortOrder = initialSort.order;
			appliedSort = { field: initialSort.field, order: initialSort.order };
			initialized = true;
		}
	};
	...
	<Popover.Root>
		<Popover.Trigger class="flex items-center text-nowrap rounded-lg border p-2 px-4">
			{#snippet child({ props })}
				...
				>
					<TablerArrowsSort class="h-4 w-4" />
